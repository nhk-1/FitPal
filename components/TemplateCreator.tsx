  ...
  return (
    <div className="px-5 pt-12 pb-24 max-w-2xl mx-auto">
      <div className="flex justify-between items-center mb-8">
        ...
        <button onClick={handleSave} className="bg-[#0a84ff] text-white px-6 py-2.5 rounded-full font-bold text-sm shadow-xl shadow-[#0a84ff]/20 active:scale-95 transition-all">Sauvegarder</button>
      </div>

      <div className="space-y-10">
        <div className="space-y-4 px-1">
          <label className="text-[11px] font-black text-[#8e8e93] uppercase tracking-widest ml-1">Nom du modèle</label>
          <input 
            type="text" 
        ...
        <div className="space-y-6">
          <div className="flex justify-between items-center px-1">
            <h2 className="text-[11px] font-black text-[#8e8e93] uppercase tracking-widest">Exercices prévus</h2>
            <button 
              onClick={() => setIsAddingExercise(true)}
              className="text-[#0a84ff] text-xs font-black flex items-center bg-[#0a84ff]/10 py-2.5 px-4 rounded-full active:scale-95 transition-all"
            >
              <Icons.Add className="w-4 h-4 mr-1.5" /> Ajouter
            </button>
          </div>

          <div className="space-y-5">
            {exercises.length === 0 && (
              <div className="bg-[#1c1c1e]/50 border-2 border-dashed border-white/5 rounded-[2.5rem] p-12 text-center">
                <p className="text-[#8e8e93] text-sm font-medium">Aucun exercice pour le moment.</p>
              </div>
            )}
            ...
            {exercises.map((ex, index) => {
              ...
              return (
                <div key={ex.id} className="bg-[#1c1c1e] border border-white/5 rounded-[2.2rem] p-6 shadow-sm">
                  <div className="flex justify-between items-start mb-6">
                    ...
                    <button onClick={() => removeExercise(ex.id)} className="text-[#8e8e93] hover:text-[#ff453a] p-2.5 bg-white/5 rounded-full active:scale-90 transition-all">
                  ...
                  <div className="grid grid-cols-4 gap-3">
                    <div className="space-y-2">
                      ...
                      <input 
                        ...
                        className="w-full bg-black/50 border border-white/10 rounded-2xl p-3.5 text-center text-sm font-black focus:border-[#0a84ff] focus:outline-none"
                      />
                    </div>
                    <div className="space-y-2">
